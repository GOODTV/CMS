...
public partial class CaseMgr_ConsultQry : BasePage 
{
    ...
    private void LoadFormData()
    {
            ...
            string strSql = @"
                            select Distinct  a.uid , a.CName AS 姓名,(select count(*) from Consulting where MemberUID=a.uid and isnull(IsDelete, '') != 'Y') as 已諮詢次數,a.Memo as 備註 ,a.Phone AS 電話
                            from Member a  
       ...
       // dr = dt.Rows[0];
        ...
        col = new NPOGridViewColumn("已諮詢次數"); //產生所需欄位及設定 Title
        col.ColumnType = NPOColumnType.NormalText; //設定為純顯示文字
        col.ColumnName.Add("已諮詢次數"); //使用欄位名稱
